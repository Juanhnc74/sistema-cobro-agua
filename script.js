...
async function loadData() {
  try {
    ...
    toast('Error al cargar datos. Verifica la conexión con el servidor.');
    return false;
  }
}
...
$('btnLogin').addEventListener('click', async ()=>{
  ...
  // Validación básica
  if (!u || !p) {
    $('loginMsg').textContent = 'Usuario y contraseña son requeridos';
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (dataLoaded) {
        ...
        $('loginMsg').textContent = 'Error al cargar datos';
      }
    } else {
...
async function renderStats(){
  ...
  // Recaudado del mes corriente - DESDE EL BACKEND
  try {
    const response = await fetch('get_total_mes.php');
    ...
    if (data.success) {
      ...
      const localTotal = payments.filter(p => p.date_iso && p.date_iso.slice(0,7) === ym)
                               .reduce((acc, p) => acc + Number(p.total||0), 0);
      $('statMonth').textContent = money(localTotal);
      $('panelMonth').textContent = money(localTotal);
    }
  } catch (error) {
...
function monthsOwed(userNum){
  ...
  for(const p of payments.filter(p=>p.user_num===userNum && p.year>=y-1)){
...
async function onRegister(e){
  ...
  try {
    const newUser = await apiRequest('users.php', {
      method: 'POST',
      body: JSON.stringify({ num, name, calle, tel, tarifa })
    });
...
function renderManageList(){
  console.log('Renderizando lista de gestión...');
  ...
  console.log('Lista de gestión renderizada');
}

async function showUserDetail(num){
  ...
  $('btnDeleteUser').onclick = async ()=>{
    ...
    try {
      await apiRequest(`users.php?num=${u.num}`, {
        method: 'DELETE'
      });
...
async function doPay(){
  ...
  try {
    const newPayment = await apiRequest('payments.php', {
      method: 'POST',
      body: JSON.stringify({
        folio,
        userNum: selectedUser.num,
        dateISO,
        year,
        months,
        rate,
        total,
        hydrant: includeHydrant
      })
    });
  ...
  document.querySelectorAll('#histBody tr[data-folio]').forEach(tr=>{
    ...
    tr.querySelector('[data-act="del"]').addEventListener('click', async ()=>{
      ...
      if(confirm('¿Eliminar este pago?')){
        try {
          await apiRequest(`payments.php?folio=${folio}`, {
            method: 'DELETE'
          });
          
          payments = payments.filter(p => p.folio !== folio);
          renderHistory();
          renderStats();
          toast('Pago eliminado');
        } catch (error) {
          toast('Error al eliminar pago');
        }
      }
    });
  });
